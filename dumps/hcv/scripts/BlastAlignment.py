...
class BlastAlignment:
	def __init__(self, query_fasta, db_fasta, tmp_dir, output_file, is_segmented_virus, master_acc, is_update, keep_blast_tmp_dir, gb_matrix, segment_file=None):
	...
	def run_blastn(self, tmp_dir, query_file):
		command = [
			'blastn',
			'-query', query_file,
			'-db', join(tmp_dir, "DB", self.db_file_name),
			'-task', 'blastn',
			'-max_target_seqs', '1',
			'-max_hsps', '1',
			'-out', join(tmp_dir, self.output_file),
	...
	def process_non_segmented_virus(self, tmp_dir, query_fasta):
		input_file = join(tmp_dir, "query_tophits.tsv")
		query_tophit_uniq = join(tmp_dir, "query_uniq_tophits.tsv")
		...
		ref_seq_dir = join(tmp_dir, "ref_seqs")
		master_seq = join(tmp_dir, "master_seq")
	...
	def process(self):
		...
		if self.is_segmented_virus == 'Y':
			self.run_makeblastdb(self.tmp_dir)
			self.run_blastn(self.tmp_dir)
			...
			if self.is_update == 'Y':	
				blast_tmp_dir = join(self.tmp_dir, "tmp_dir")
				...
				self.run_makeblastdb(self.tmp_dir)
				self.run_blastn(self.tmp_dir, self.query_fasta)
				self.process_non_segmented_virus(self.tmp_dir, self.query_fasta)
				self.update_gB_matrix(self.query_fasta, join(self.tmp_dir, "query_uniq_tophits.tsv"), self.gb_matrix)
