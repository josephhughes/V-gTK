		...
		with open(query_seq_file, 'w') as write_query_seq, open(ref_seq_file, 'w') as write_ref_seq:
			with open(self.genbank_matrix) as file, open(join(self.genbank_matrix_filtered, 'gB_matrix.tsv'), 'w', newline='') as filtered_matrix:
				...
				for row in csv_reader:
					...
					'''
					if self.gb_division is not None and gb_division in self.gb_division:
        	exclusion_dict[accession] = {
            'exclusion_status': 1,
            'exclusion_criteria': 'excluded due to GenBank division in user exclusion list'
        	}
        	continue
        	...
        	exclusion_dict[accession] = {
            'exclusion_status': 1,
            'exclusion_criteria': 'excluded due to sequence length less than user-defined threshold'
        	}
        	continue
        	...
        	exclusion_dict[accession] = {
            'exclusion_status': 1,
            'exclusion_criteria': 'excluded due to real length (length - Ns) less than user-defined threshold'
        	}
        	continue
					'''
					if (	
...
# 1. Add individual exclusion criteria
# 2. read gB_matrix
# 3. Which ever the exclusion criteria add it along with reason in the gB_matrix
